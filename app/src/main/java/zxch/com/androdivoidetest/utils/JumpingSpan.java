...
public class JumpingSpan extends SuperscriptSpan implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void initIfNecessary(float ascent) {
        ...
        this.shift = 0;
        int maxShift = (int) ascent / 2; //设置最大字体偏移量maxshift * (0, 1)
    ...
    @Override
    public void onAnimationUpdate(ValueAnimator animation) { // No need for synchronization as this always runs on main thread anyway
    ...
    private void updateAnimationFor(ValueAnimator animation, TextView v) {
        if (isAttachedToHierarchy(v)) { //设置最大字体偏移量maxshift * (0, 1)
...
//            Log.e("gac", "animRange:" + animatedRange);
        ...
        @Override
        public float getInterpolation(float input) { // We want to map the [0, PI] sine range onto [0, animRange]
            if (input > animRange) {
                return 0f;
