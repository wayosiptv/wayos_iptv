...
public class NewBaseLiveAct extends Activity {

    private final static String TAG = "NewBaseLiveAct";
...
//        getShareMusicAd();
    ...
    private Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            mFloatService = new Intent(NewBaseLiveAct.this, TestService.class);
...
//                    leftNums = 0;
//                    rightNums = 0;
//                    Intent intent = new Intent(NewBaseLiveAct.this, DialogActivity1.class);
...
//    }
        ...
        if (SpUtilsLocal.contains(mContext, "tokenLoseTime")) {     //判断当前是否存在token失效键值
            ...
            if (!("").equals(tokenTime) && tokenTime.length() != 0) {
                ...
                if (toeknValue > 3600) {    //相差一小时后重新获取阿里Toekn
                    ...
                    //判断当前的语音服务是否启动
                    if (isServiceRunning(NewBaseLiveAct.this,
    ...
    private void getTokenToServer() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    AccessToken accessToken = new AccessToken("LTAINJ2Q07s72DhW", "qOzkzCbLnha6Hl0d8TbQhzvKMiWq1A");
                    accessToken.apply();
