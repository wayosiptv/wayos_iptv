...
class DownloadAppUtils {
    ...
    public static void download(final Context context, String url, final String serverVersionName) {
        ...
        String apkLocalPath = null;
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {//外部存储卡
            filePath = Environment.getExternalStorageDirectory().getAbsolutePath();
            apkLocalPath = filePath + File.separator + packageName + "_" + serverVersionName + ".apk";
        } else {
            Log.i(TAG, "没有SD卡");
            apkLocalPath = "/system/app/" + serverVersionName + ".apk";
            Log.e(TAG, "download filePath: " + apkLocalPath);
        }


        Log.e(TAG, "onReceive: filePath" + filePath);
        Log.e(TAG, "onReceive: apkLocalPath" + apkLocalPath);

        downloadUpdateApkFilePath = apkLocalPath;
        Log.e(TAG, "onReceive: downloadUpdateApkFilePath:" + downloadUpdateApkFilePath);
